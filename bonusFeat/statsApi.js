  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      for (var j = 0; j < response.conferences[0].divisions.length; j++) {
        for (
          var i = 0;
          i < response.conferences[0].divisions[0].teams.length;
          i++
        ) {
          let teamName = response.conferences[0].divisions[j].teams[i].name;
          let teamRank =
            response.conferences[0].divisions[j].teams[i].calc_rank.conf_rank;
      ...
      for (var j = 0; j < response.conferences[1].divisions.length; j++) {
        for (
          var i = 0;
          i < response.conferences[1].divisions[0].teams.length;
          i++
        ) {
          let teamName = response.conferences[1].divisions[j].teams[i].name;
          let teamRank =
            response.conferences[1].divisions[j].teams[i].calc_rank.conf_rank;
          console.log(teamRank, teamName);
