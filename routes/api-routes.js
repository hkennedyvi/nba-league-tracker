...
module.exports = function(app) {
  ...
  app.post("/api/saveplayer", function(req, res) {
    //first it searches the DB if the playerapiid and user id are already stored for the player being saved
    db.Player.findAll({
      where: {
      ...
      // if no match, then save the new player under the current user
      if (data.length === 0) {
        db.Player.create({
          name: req.body.name,
          api_id: req.body.api_id,
          ...
          .catch(function() {
            res.send({ error: "error saving player" });
          });
      } else {
